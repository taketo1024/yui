//! One-shot run of `KhComplex::new` (non-sym, FF2) on the 60-crossing knot —
//! the underlying link of the strongly invertible target used in profile_khi_60.
//!
//! Usage:
//! ```
//! RUST_LOG=info cargo run --release --example profile_kh_60          # greedy
//! RUST_LOG=info SELECTIVE=1 cargo run --release --example profile_kh_60   # selective
//! H_RANGE=-2..=2 ...                                                 # restrict the build
//! ```
...
fn main() {
    ...
    let il = InvLink::from_symmetric_pd_code(pd.iter().copied());
    let l = il.inner();
