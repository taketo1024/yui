//! [`Summand<X, R>`]: a single graded piece of a chain complex, modeling a
//! finitely-generated R-module with named generators.
...
/// A finitely-generated R-module summand: stores raw generators of type `X`,
/// the free rank, torsion coefficients, and a [`Trans`] recording the basis
...
where X: LcKey, R: Ring, for<'x> &'x R: RingOps<R> {
    ...
    /// the SNF basis. Terms whose key is not in this summand are silently
    /// dropped.
    ...
    /// Like [`Self::vectorize`], but reduces torsion-class coordinates modulo
    /// their respective orders. Useful when the resulting vector represents an
    /// element of the homology and you want a canonical representative.
